...
def csvwriter(set1,df,url):
	...
	fd = pd.DataFrame(list(zip(list1,list2,list3)),columns=['hash','date','source'])
	result = pd.concat([df,fd],ignore_index=True)
	result = result.drop_duplicates('hash',keep='first')
	return result
